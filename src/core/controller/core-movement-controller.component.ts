...
export class CoreMovementControllerComponent {
   ...
   constructor(
      ...
      keyboardController.keys$.subscribe(
         () => (this.keyDirection = this.getKeyDirection())
      );
      cameraController.quaternion$.subscribe(() =>
         this.lookingDirection.copy(cameraController.getDirection())
      );
   ...
   private getKeyDirection(): KeyDirection {
      if (
         this.keyboardController.isPressed(
            CoreMovementControllerComponent.KEY_FORWARD
         ) &&
         this.keyboardController.isPressed(
            CoreMovementControllerComponent.KEY_LEFT
         )
      )
      ...
      if (
         ...
         this.keyboardController.isPressed(
            CoreMovementControllerComponent.KEY_RIGHT
         )
      )
      ...
      if (
         this.keyboardController.isPressed(
            CoreMovementControllerComponent.KEY_BACKWARDS
         ) &&
         this.keyboardController.isPressed(
            CoreMovementControllerComponent.KEY_LEFT
         )
   ...
   private step(delta: number): void {
      if (this.keyDirection !== KeyDirection.IDLE) {
         ...
         const additionalAngle =
            this.keyDirection * CoreMovementControllerComponent.DEG_45 +
            CoreMovementControllerComponent.DEG_180;
