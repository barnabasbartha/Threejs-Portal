...
class Main {
   ...
   private async initThread<T>(
      worker: WorkerImplementation
   ): Promise<[T, WorkerImplementation]> {
   ...
   private initCoreThread(): void {
      ...
      this.initThread<CoreThread>(new Worker("../core/core-thread")).then(
         ([coreThread, coreWorker]) => {
            this.coreThread = coreThread;
            console.log("Core thread created");
            this.initComponents();
            coreWorker.postMessage({canvas: offscreenCanvas}, [
               offscreenCanvas,
            ]);
         }
      );
